...
theme.init = function () {
  ...
  if (!theme.cache.$body.hasClass('coffee-product')) {
  ...
  //   function ajaxchimpCallbackModal (resp) {
  ...
  //   $('#mc-embedded-subscribe-form-modal').ajaxChimp({
  //       url: 'http://deathwishcoffee.us10.list-manage.com/subscribe/post?u=8f096b48f8389f5758508e133&amp;id=b9a1564620',
  //       callback: ajaxchimpCallbackModal
  //   });
  // }
};
...
theme.sizeCartDrawerFooter = function () {
  ...
  // added to remove gift product when the promo has ended
      ...
      $.magnificPopup.open({
        ...
        callbacks: {
          close: function() {
            ...
            Cookies.set('popup', 'true', { expires: date });
          }
        }
      });
     }, 5000);
...
theme.sbbgPopup = function() {
  ...
  if (cookie == undefined){
    ...
    $(document).on('click', '.vote__submit', function (e) {
      var date = new Date();
      var minutes = 2;
      date.setTime(date.getTime() + (minutes * 60 * 1000));
      Cookies.set('popup', 'true', { expires: date });
...
theme.collectionParallax = function () {
  ...
  // $(".header-wrapper").addClass('hero__header').prependTo(".collection-hero");
...
theme.createImageCarousel = function () {
  if (!theme.cache.$productImages.length || theme.cache.$productImagePhoto.length < 2 || theme.cache.$body.hasClass('coffee-product')) {
    return;
  }
...
theme.initStickyProductMeta = function () {
  ...
  if (theme.cache.$productReturnLink.length) {
    calcLimit = theme.cache.$productReturnLink.offset().top - 190;
...
theme.faqAccordion = function () {

  if (!theme.cache.$accordion.length) {
    return;
  }

  theme.cache.$accordion.click(function(e) {
     e.preventDefault();
     var parent = $(this).parent('li');
     $('.accordion li').not(parent).removeClass('is-visible');
